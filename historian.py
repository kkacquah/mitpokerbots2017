...
stats['V_FoldCount'] = 0
stats['AverageRaiseAll'] = 0.0
stats['StdDevRaiseAll'] = 0.0
stats['TotRaiseAll'] = []
stats['CountRaiseAll'] = 0
stats['HigherRaiseRate'] = 0.0
stats['HighRaiseRate'] = 0.0
stats['HigherRaiseCount'] = 0
stats['HighRaiseCount'] = 0

stats['AverageBetAll'] = 0.0
stats['StdDevBetAll'] = 0.0
stats['TotBetAll'] = []
stats['CountBetAll'] = 0
stats['HigherBetRate'] = 0.0
stats['HighBetRate'] = 0.0
stats['HigherBetCount'] = 0
stats['HighBetCount'] = 0
...
def update(lastActions,myName,oppName):
	...
	for round in lastActions:
		if (round[0] == 'RAISE' and round[-1] == oppName):
			stats['TotRaiseAll'].append(int(round[1]))
			stats['CountRaiseAll'] += 1
	for round in lastActions:
		if (round[0] == 'BET' and round[-1] == oppName):
			stats['TotBetAll'].append(int(round[1]))
			stats['CountBetAll'] += 1
	#Flop C_BET count
	for i in range(len(lastActions)-1):
	...
	for i in range(len(lastActions)-1):
		if lastActions[i] == ['DEAL','TURN']:
			...
			break
	try:
   		stats['AverageBetAll'] = sum(stats['TotBetAll'])/float(stats['CountBetAll'])
   	except ZeroDivisionError:
   		stats['AverageBetAll'] = 0.0
   	try:
   		stats['AverageRaiseAll'] = sum(stats['TotBetAll'])/float(stats['CountRaiseAll'])
   	except ZeroDivisionError:
   		stats['AverageRaiseAll'] = 0.0
	squarevariance = 0
	for x in stats['TotBetAll']:		
			squarevariance += ((x - stats['AverageBetAll'])**2)
	try:
		stats['StdDevBetAll'] = math.sqrt(squarevariance/len(stats['TotBetAll']))
	except ZeroDivisionError:
		stats['StdDevBetAll'] = 0
	for x in stats['TotRaiseAll']:		
			squarevariance += ((x - stats['AverageRaiseAll'])**2)
	try:
		stats['StdDevRaiseAll'] = math.sqrt(squarevariance/len(stats['TotRaiseAll']))
	except ZeroDivisionError:
		stats['StdDevBetAll'] = 0
	for round in lastActions:
	...
	for round in lastActions:
		if (round[0] == 'RAISE' and round[-1] == oppName):
			if (int(round[1]) >= stats['StdDevRaiseAll']):
				stats['HighRaiseCount'] += 1
				stats['HighRaiseRate'] = stats['HighRaiseCount']/float(stats['numHandsPlayed'])
	for round in lastActions:
		if (round[0] == 'RAISE' and round[-1] == oppName):
			if (int(round[1]) >= stats['StdDevRaiseAll']*1.5):
				stats['HigherRaiseCount'] += 1
				stats['HigherRaiseRate'] = stats['HigherRaiseCount']/float(stats['numHandsPlayed'])
	for round in lastActions:
		if (round[0] == 'BET' and round[-1] == oppName):
			if (int(round[1]) >= stats['StdDevBetAll']):
				stats['HighBetCount'] += 1
				stats['HighBetRate'] = stats['HighBetCount']/float(stats['numHandsPlayed'])
	for round in lastActions:
		if (round[0] == 'BET' and round[-1] == oppName):
			if (int(round[1]) >= stats['StdDevBetAll']*1.5):
				stats['HigherBetCount'] += 1
				stats['HigherBetRate'] = stats['HigherBetCount']/float(stats['numHandsPlayed'])
	try:
