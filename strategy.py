...
def getAction(myHand,boardCards,legalActions,FullLastActions,lastActions,history,switched,button,oppName):
    ...
    if hasAction("DISCARD",legalActions) != -1: #means at flop or turn and can discard card
        ...
        pastRaises = lastRaises(FullLastActions[:],lastActions[PIPindex])
        print 'raises are : ' + str(pastRaises)
        
        changeRaise = pastRaises[1] - pastRaises[0]
        ...
        if (changeRaise > 25 or pastRaises[1] > ((prob * 2)*.85) ):
            return 'FOLD\n'
        ...
        if round == ['DEAL','FLOP']:
            for j in range(i,(len(lastActions))):
                if lastActions[j][0] == 'BET' and lastActions[j][-1] == oppName:
                    aggressiveEventsOccurred.append('V_FlopCbetRateWA')           
            break
    for i in range(len(lastActions)-1):
        if round == ['DEAL','TURN']:
            for j in range(i,(len(lastActions))):
                if lastActions[j][0] == 'BET' and lastActions[j][-1] == oppName:
                    aggressiveEventsOccurred.append('V_TurnCbetRateWA')            
            break
...
def lastRaises(FullLastActions,VPIPaction):
    VPIP = VPIPaction.split(':')
    newRaise = VPIP[1]
    newRaise = 0
    oldRaise = 0
    for i in range(len(FullLastActions)-1,0,-1):
        ...
        if  action.find("RAISE") != -1 or action.find("BET") != -1 or  action.find("POST") != -1:
                if newRaise == 0:
                    newRaise = action.split(':')[1]
                    continue
                oldRaise = action.split(':')[1]
                break
